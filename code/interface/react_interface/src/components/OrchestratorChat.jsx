...
import { MessageSquare, Send, Loader2, Sparkles, PlugZap, Layers, Target, RefreshCcw } from 'lucide-react';
...
const OrchestratorChat = () => {
    ...
    return (
        <div className="flex flex-col h-full">
            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                {history.length === 0 && (
                    <div className="text-center py-12">
                        ...
                        <h2 className="text-2xl font-bold text-white mb-2">Orchestrator Console</h2>
                        <p className="text-gray-400 max-w-xl mx-auto">
                            Talk naturally. The orchestrator will extract intent/entities, route to the right agents, and respond using your chosen LLM provider.
                        </p>
                    </div>
                )}
            ...
            <div className="border-t border-dark-border p-6 space-y-4 bg-dark-bg/70 backdrop-blur-md">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-3">
                        <div className="flex items-center gap-3">
                            <label className="text-xs uppercase tracking-wider text-gray-500">Provider</label>
                            <select
                                value={provider}
                                onChange={(e) => setProvider(e.target.value)}
                                className="bg-dark-panel border border-dark-border text-sm rounded-lg px-3 py-2"
                            >
                                {PROVIDERS.map(item => (
                        ...
                        <input
                            ...
                            placeholder="Primary API key (per conversation)"
                            className="w-full bg-dark-panel border border-dark-border rounded-lg px-4 py-2 text-sm"
                        />
                        <div className="flex gap-3">
                            <input
                                value={modelOverride}
                                onChange={(e) => setModelOverride(e.target.value)}
                                placeholder="Responder model override (optional)"
                                list="model-options"
                                className="flex-1 bg-dark-panel border border-dark-border rounded-lg px-3 py-2 text-sm"
                            />
                            <input
                                value={plannerModelOverride}
                                onChange={(e) => setPlannerModelOverride(e.target.value)}
                                placeholder="Planner model override (optional)"
                                list="model-options"
                                className="flex-1 bg-dark-panel border border-dark-border rounded-lg px-3 py-2 text-sm"
                            />
                    ...
                    <div className="space-y-3">
                        <textarea
                            ...
                            className="w-full bg-dark-panel border border-dark-border rounded-lg px-4 py-2 text-sm h-[86px] resize-none"
                        />
                        <input
                            value={theoryConversationId}
                            onChange={(e) => setTheoryConversationId(e.target.value)}
                            placeholder="Theory Specialist conversation ID (optional)"
                            className="w-full bg-dark-panel border border-dark-border rounded-lg px-4 py-2 text-sm"
                        />
                        <div className="flex items-center gap-2 text-xs text-gray-500">
                            ...
                            <input
                                ...
                                className="flex-1 bg-dark-panel border border-dark-border rounded-lg px-3 py-2 text-xs"
                            />
                            <label className="flex items-center gap-2">
                                <input
                                    type="checkbox"
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-3 text-xs">
                    {Object.keys(extraKeys).map(key => (
                        <input
                            ...
                            placeholder={`${key} key (optional)`}
                            className="bg-dark-panel border border-dark-border rounded-lg px-3 py-2"
                        />
                    ))}
                </div>
                ...
                <div className="flex gap-4">
                    ...
                    <button
                        ...
                        className="bg-dark-panel border border-dark-border text-gray-300 rounded-xl px-4 hover:border-neon/50 transition-all duration-300 flex items-center justify-center"
                    >

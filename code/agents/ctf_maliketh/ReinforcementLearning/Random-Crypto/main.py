...
def generate_story_using_llm(challenge_data):
    ...
    """
    try:
        print(f"[DEBUG] Calling Ollama API for {challenge_data.get('archetype', 'unknown')}:{challenge_data.get('subtype', 'unknown')}")
        print(f"[DEBUG] API URL: {OLLAMA_API_URL}")
        print(f"[DEBUG] Model: {OLLAMA_MODEL}")
        print(f"[DEBUG] API Key present: {bool(OLLAMA_API_KEY)}")
        ...
        print(f"[DEBUG] Raw response text (first 200 chars): {response_text[:200]}...")

        story_text = re.findall(r"<challenge>(.*?)<\/challenge>", response_text, re.DOTALL)[0]
        story_text = story_text.replace("<CIPHER>", challenge_data["ciphertext"])
        print(f"[DEBUG] Extracted story text successfully")
        return story_text
    ...
    for i in range(variants):
        for archetype in SUBTYPES:
            for subtype in SUBTYPES[archetype]:
                print(f"\n[INFO] Generating {archetype}:{subtype}")
...
def main():
    ...
    print(f"[INFO] Starting challenge generation with {args.variants} variant(s)")
    print(f"[INFO] Output folder: {args.output_folder}")
    print(f"[INFO] Total challenges to generate: {args.variants * sum(len(v) for v in SUBTYPES.values())}")

    challenges = generate_every_possible_challenge(variants=args.variants, output_folder=args.output_folder)

    print(f"\n[INFO] Finished generating {len(challenges)} challenges.")

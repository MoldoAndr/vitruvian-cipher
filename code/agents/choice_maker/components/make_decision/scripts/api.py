...
def _json_error(message: str) -> None:
    """Raise a payload error with a message."""
...
def process_payload(
    ...
    """Validate the payload and run the requested inference."""
    ...
    if operation == "entity_extraction":
        result = handle_entity_extraction(text, entities_model)
    else:
        result = handle_intent_extraction(text, intent_model)
...
def main() -> None:
    ...
    try:
        ...
        response = process_payload(payload, args.entities_model, args.intent_model)
